...
export function FilterBadge({ label, filterType, variant = "genre", className = "" }: FilterBadgeProps) {
  const router = useRouter()

  // Determine the style based on label content for game modes
  let style = variantStyles[variant]
  if (variant === "gameMode") {
    if (label.toLowerCase().includes("single")) {
      style = variantStyles.gameMode
    } else if (label.toLowerCase().includes("co-op") || label.toLowerCase().includes("coop")) {
